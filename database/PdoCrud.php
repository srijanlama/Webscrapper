...
class PdoCrud
{
    ...
    function buildKeys($record){
        $new_values = [];
        foreach(array_keys($record) as $value){
            array_push($new_values, '`'. $value. '`');
        }
    ...
    public function insertOrUpdate(string $tableName, array $record): int
    {
        $fields = $this->buildKeys($record);
        $values = $this->buildValues($record);

        $query = 'INSERT INTO ' . $tableName . ' (' . $fields . ') VALUES ('. $values .') ON DUPLICATE KEY UPDATE ' . $this->setQuery($record);
        echo $query;
        exit;
        $result = $this->query($query);
